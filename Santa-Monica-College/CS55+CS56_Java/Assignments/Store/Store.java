...
public class Store {   
     ...
     */
    //store size is set as final
    public static final int MAX_CAPACITY = 75;    
    public static ShoppingCart[] carts;         //array of objects of type Shoppingcart
    public static Customer[] customers;         //array of objects of type Customer
...
//            customers[i].assignCart();
    ...
    public static void main(String[] args) {
        ...
        while(true) {
        System.out.println("\nSTORE MENU");
        System.out.println("(1) Assist Customers (e.g. add/remove items to cart)\n(2) View customer cart \n(3) Add more customers (is rarely needed)\n(4) Use checkout counter\n(5) Show all customers \n(6) Show active shopping carts \n(7) Shopping cart interface (preview...not functional)");    
        
        //The switch statement provides navigation
        //different cases rely on object instances and instant methods for accessing and mutating
        switch(input.next()) {
            case("7"):
                System.out.println("\nCART TRACKER:");
            ...
            case("4"):
                ...
                cu.removeCart();
                customers[getCustomerIndex(customers, custID)] = null;
                //customers[getCustomerIndex(customers, cust.getCustomerID())]=null;                
...
//                    break;}
            ...
            case ("2"):
                ...
                break;
            case("1"):
                System.out.println("STORE MENU: CUSTOMER TRACKER");                
                System.out.println("Enter a Customer ID from below: ");
                System.out.println("(You may have to scroll in order to view all 75 customers...recall customers have already been generated automatically.)");
                ...
                switch(input.nextInt()) {
                    case(1):
                        ...
                        do {
                            System.out.println("Which kind of item would you like to add? (b) Book, (m) Movie, or (c) CD, you can also add prefix - for removing items so -b, -m, -c are valid as well");
                            System.out.println("Additionally since items are often avaialable in discount (books and cds only, no discounted movies for sale), so add a 'D' at the end bD or cD  are also valid");
    ...
    //The general method for showing customers WITH ALL OTHER OVERLOADED METHODS FOLLOWING
...
//            System.out.print(", " + a[i].customerID);
    ...
    public static void showCustArray(Customer[] a, String m, String n) {
        int count = 0;
        for (int i =0; i<Store.customers.length; i++) {
            if(a[i].getCart()!=null) {
                count++;
    ...
    public static void showCustArray(Customer[] a, String j, String k, String l) {
        for (int i =0; i<Store.customers.length; i++) {
            if(a[i]==null) continue;
            System.out.print(a[i].getCustomerID()+",");            
        }
        System.out.println();
    }
    
    //A very useful way of accessing the carts of customers through the overloaded method
    public static void showCustArray(Customer[] a, int index) {
...
//    }
    ...
    public static int getLastCustomerIndex (Customer[] array) {
        for (int i = 0; i<Store.customers.length; i++) {
            if(array[i]==null)
                return i;
        }
    ...
    public static String getCustomerLocation(Customer[] array, int i) {
        return array[i].getCustomerID();
    }

    public static Customer assignToCustomer(Customer[] array, String id) {
    ...
    public static int getLinePlacement(Customer[] a, Customer cust) {
        int lineCount = 1;
        System.out.println("Customers with carts form the line: ");
        for (int i =0; i<Store.customers.length; i++)
        {
            if(a[i].getCart()==null) continue;
            lineCount++;
            if(a[i]!=cust) continue;
        }
     return lineCount;
        }   

    public static int getCustomerIndex(Customer[] array, String id) {
...
//        getCart().isNotInUse();
    ...
    public void assignCart() {
        for(int i = 0; i<Store.carts.length; i++) {
            if (Store.carts[i].isCartAvailable()==false) {
                this.cartObjectRef = Store.carts[i];
                Store.carts[i].isInUse();
                break;
            } 
            else
